...
static int t=0, a=0, z=1, x=0, count1=0, count2=0, c=0, i=0, y=0;	//inicializaÁ„o das vari·veis
static int score=0;
static int flag=0;
...
void cenario(int escolha){
	...
	int a=3,b=4,c=2,d=1,e=0;
    			...
    			for(int i=0; i<28; i++){ //RepetiÁıes dos predios
...
void timer(int value){ //Estudar 
	...
	else if(a==30&&flag==5)
	{
		if(y==0)
		{
			MessageBox(0,"You Win √µ/","Win",MB_OK|MB_ICONINFORMATION);
			exit(0);
		}
		else
		{
...
void keyboard (unsigned char key, int x, int y){	//Adcionar mais funÁıes
...
void MenuInformacaoes(int op){ //InformaÁıes do Menu
	
   switch(op) { //SeleÁ„o do menu
            ...
            case 1:
                    ...
                    break;
            case 2:
                    MessageBox(0,"Chegar atÈ o final sem bater nos canos","Regras",MB_OK|MB_ICONINFORMATION);
                    break;
    }
} 
...
int menu,submenuFundo,submenuInfo,submenuJanela;
	...
	glutAddSubMenu("InformaÁıes",submenuInfo);
...
void GerenciaMouse(int button, int state, int x, int y){ //Gerenciamento do Mouse
    ...
    if (state == GLUT_DOWN) {
        MenuMouse(); //Chamada da funÁ„o menu mouse
	}
...
int main(){
	...
	glutInitWindowSize (800, 600); //ResoluÁ„o
	glutInitWindowPosition (600, 250); //PosiÁ„o Inicial
	glutCreateWindow("Flappy C Bird"); //Nome da janela
	glutReshapeFunc(AlteraTamanhoJanela);//MaximizaÁ„o
